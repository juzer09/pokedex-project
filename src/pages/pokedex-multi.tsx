...
const PokedexPage: React.FC = () => {
    ...
    const { data: pokemons, isLoading, error } = trpc.pokemon.getMultiplePokemon.useQuery(
    ...
    useEffect(() => {
        if (pokemons && resultsRef.current) {
    ...
    return (
        <>
            <Head>
                <title>Multi Search - Gotta Know &apos;Em All Pokédex</title>
                        ...
                        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 4 }} id='hm__search'>
                            <Box sx={{ mb: 2 }} className='hm__search-box'>
                                <TextField
                                    fullWidth
                                    variant="outlined"
                                    placeholder="Pokemon Names (comma-separated)"
                                    value={pokemonNames}
